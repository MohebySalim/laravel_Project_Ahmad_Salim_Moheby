...
@section('content')
    <div class="page-content">
            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{__('perpetrators') }}</li>
                    </ol>
                </nav>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">{{__('perpetrators') }}</h6>
                <hr>
                <div class="accordion accordion-flush" id="accordionFlushExample">
                    <form action="{{ route('perpetrators.store') }}" method="post" enctype="multipart/form-data" autocomplete="off" class="needs-validation" novalidate>
                        @csrf
                    ...
                    <div class="float-end">
                        @if(request('search'))
                            <div class="tooltip1">
                                <a href="{{ route('perpetrators.index') }}" class="btn btn-primary" ><i class="bx bx-reset"></i><span class="tooltiptext2">{{__('reset') }}</span></a>
                            </div>
                        @endif
                        <div class="tooltip1">
                            <a href="{{ route('perpetratorExport') }}" class="btn btn-primary" ><i class="bx bx-export"></i><span class="tooltiptext2">{{__('export') }}</span></a>
                        </div>
                    </div>

                    <form action="{{ route('perpetrators.index') }}" method="get" autocomplete="off">
                        @csrf
                        <div class="row">
                            <div class="col-md-12 mb-2">
                                <div class="input-group">
                                    <input type="text" class="form-control search-control" name="search" placeholder="Type Perpetrator Code and enter..." required><span class="position-absolute top-50 search-show translate-middle-y"><i class="bx bx-search"></i></span>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="d-flex justify-content-center order-actions">
                                    ...
                                    <form id="delete-form-{{ $item->id }}" method="post" action="{{ route('perpetrators.destroy', $item->id) }}" style="display: none;">
                                        @csrf
                                ...
                                <div class="modal-dialog modal-lg modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h1 class="modal-title fs-5" id="exampleModalLabel">{{__('editPerpetrator') }}</h1>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <form action="{{ route('perpetrators.update', $item->id) }}" method="post" enctype="multipart/form-data" autocomplete="off" class="needs-validation" novalidate>
                                                @csrf
                                                @method('PUT')
                                                <div class="form-group p-4 row">
                                                    <div class="col-md-3">
