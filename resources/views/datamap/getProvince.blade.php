...
<section class="centered-container toggle eng"><a class="link link--arrowed" href="#"><span id="targetit">Select Year</span>
...
<div class="cont">
    ...
    <ul class="graphs stats-container centered biggie">
        @foreach($violences as $violence)
            <li class="animated" data-provide="circular" data-fill-color="#7C0D0D" data-percent="true" data-initial-value="{{ $violence->caseCount }}" data-max-value="100"  data-label="{{ (app()->getLocale() === 'dr') ? $violence->name_dr :
            ((app()->getLocale() === 'ps') ? $violence->name_ps :
            ((app()->getLocale() === 'en') ? $violence->name_en : '')) }}" data-title="{{__('total_case')}}: {{ $violence->caseCount }}"
                data-dates="@if( request('year') ){{ $year }}@else{{ (app()->getLocale() === 'dr') ? 'سال: همه' :
                    ((app()->getLocale() === 'ps') ? 'کال: ټول' :
                    ((app()->getLocale() === 'en') ? 'Year: All' : '')) }}@endif" style="position: relative; width: 150px; height: 100%;">
            </li>
        @endforeach
    ...
    <div class='app-layout'>
        @foreach($perpetrators as $perpetrator)
            <div class='box gov'><span data-localize="gov">{{ (app()->getLocale() === 'dr') ? $perpetrator->name_dr :
            ((app()->getLocale() === 'ps') ? $perpetrator->name_ps :
            ((app()->getLocale() === 'en') ? $perpetrator->name_en : '')) }}</span>

                <div class="dwn"><img src="{{ asset('datamap/images/arr.png') }}" style="width: 20px;height: 30px;">
                    <p>
                        {{ $year }}
                        <br/>
                        {{ $perpetrator->ajsc_cases_count }}
                        <br/>
                        @foreach($perpetrator->ajscCases as $ajscCase)
                            @foreach($ajscCase->violences as $violence)
                                {{ (app()->getLocale() === 'dr') ? $violence->name_dr :
                                    ((app()->getLocale() === 'ps') ? $violence->name_ps :
                                    ((app()->getLocale() === 'en') ? $violence->name_en : '')) }} : {{ $violence->ajscCases_count }}
                            @endforeach
                        @endforeach
                    </p>
                    <hr/>
    ...
    <table class="w3-table-all" id="myTable">
        ...
        @foreach($results as $result)
            <tr>
                ...
                <td>@foreach($result->perpetrators as $perpetrator){{ (app()->getLocale() === 'dr') ? $perpetrator->name_dr :
            ((app()->getLocale() === 'ps') ? $perpetrator->name_ps :
            ((app()->getLocale() === 'en') ? $perpetrator->name_en : '')) }}@endforeach</td>
                <td>@foreach($result->violences as $violence){{ (app()->getLocale() === 'dr') ? $violence->name_dr :
            ((app()->getLocale() === 'ps') ? $violence->name_ps :
            ((app()->getLocale() === 'en') ? $violence->name_en : '')) }}@endforeach</td>
                <td>@foreach($result->occupations as $occupation){{ (app()->getLocale() === 'dr') ? $occupation->name_dr :
            ((app()->getLocale() === 'ps') ? $occupation->name_ps :
            ((app()->getLocale() === 'en') ? $occupation->name_en : '')) }}@endforeach</td>
                <td>{{ (app()->getLocale() === 'dr') ? $result->province->name_dr :
            ((app()->getLocale() === 'ps') ? $result->province->name_ps :
            ((app()->getLocale() === 'en') ? $result->province->name_en : '')) }}</td>
                <td>{{ (app()->getLocale() === 'dr') ? $result->zone->zone_dr :
            ((app()->getLocale() === 'ps') ? $result->zone->zone_ps :
            ((app()->getLocale() === 'en') ? $result->zone->zone_en : '')) }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
